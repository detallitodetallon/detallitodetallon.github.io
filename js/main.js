...
const handleProducto = () => {
    ...
    productos.forEach(producto => {
        producto.addEventListener('click', handleProductoSeleccionado);
    });
}
...
// const handleProductoSeleccionado = (event) => {
//     const producto = event.currentTarget;
//     const modalId = producto.getAttribute('data-bs-target');
//     const modalElement = document.querySelector(modalId);

//     const itemProducto = modalElement.querySelector('.modal-title');
//     const precioProducto = modalElement.querySelector('.modal-precio');
...
async function fetchSheetData(sheetName, range) {
    ...
    const API_KEY = 'AIzaSyCOOZyqVXqOkAP4clbJjn6YBY9c3tcovnc';  
...
function transformarProducto(row) {
    return {
        ...
        sku: row[8],
        destacado: row[9] ? row[9].toLowerCase() === 'true' : false,
        destacado: row[10] ? row[10].toLowerCase() === 'true' : false,
...
async function fetchArreglos() {
    const data = await fetchSheetData('Copia de Arreglos', 'A:J');
    const products = data.map(transformarProducto).filter(product => product.id);
    return products;
}
...
async function fetchGigantes() {
    const data = await fetchSheetData('Gigantes', 'A:J');
    const products = data.map(transformarProducto).filter(product => product.id);
    return products;
}
...
async function fetchHelios() {
    const data = await fetchSheetData('Helios', 'A:J');
    const products = data.map(transformarProducto).filter(product => product.id);
    return products;
}
...
async function fetchFlores() {
    const data = await fetchSheetData('Flores', 'A:J');
    const products = data.map(transformarProducto).filter(product => product.id);
    return products;
}
...
/* harmony import */ var _urls_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(7);
/* harmony import */ var _urls_js__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_urls_js__WEBPACK_IMPORTED_MODULE_4__);
/* harmony import */ var _slider_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(8);
